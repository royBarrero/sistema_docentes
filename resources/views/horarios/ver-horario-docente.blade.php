    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-info text-white">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('horarios.consultar.docente') }}" class="btn btn-sm btn-light">
                    <i class="bi bi-arrow-left"></i> Cambiar Docente
                </a>
            </div>
        </div>
        <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <h5 class="mb-0">
                            ...
                            <strong>Docente:</strong> {{ $docente->usuario->nombre_completo ?? 'N/A' }}
                        </h5>
                        @if($docente->usuario->email)
                            <small class="text-muted">
                                <i class="bi bi-envelope"></i> {{ $docente->usuario->email }}
                            </small>
                        @endif
                    </div>
                </div>
            ...
            @if($horarios->isEmpty())
                <div class="alert alert-warning text-center">
                    ...
                    <h5>Este docente no tiene horarios asignados</h5>
                    <p class="mb-0">Asigna horarios usando el asistente o manualmente.</p>
                </div>
                <div class="text-center">
                    <a href="{{ route('horarios.asistente') }}" class="btn btn-primary">
                        <i class="bi bi-magic"></i> Generar Horarios
                    </a>
                </div>
            @else
                <!-- Gestión académica -->
                                        ...
                                        <td class="{{ $horarios_encontrados->isNotEmpty() ? 'table-info' : '' }}">
                                            @if($horarios_encontrados->isNotEmpty())
                                                @foreach($horarios_encontrados as $horario)
                                                    <div class="mb-2">
                                                        <strong class="d-block">{{ $horario->grupo->materia->nombre ?? 'N/A' }}</strong>
                                                        <small class="text-muted d-block">
                                                            Grupo: {{ $horario->grupo->nombre ?? 'N/A' }}
                                                        </small>
                                                        <small class="text-muted d-block">
                                                            <i class="bi bi-door-open"></i> {{ $horario->aula->codigo ?? 'N/A' }}
                                                        </small>
                                                        <small class="text-muted">
            ...
            <div class="text-center mt-4">
                <a href="{{ route('horarios.consultar.docente') }}" class="btn btn-primary">
                    <i class="bi bi-arrow-left"></i> Consultar Otro Docente
                </a>
